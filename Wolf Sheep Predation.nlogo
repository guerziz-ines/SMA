...
to setup
  ...
  [
    ask patches [ set pcolor one-of [green red ] ]
  ]
  ...
  create-redwolves initial-number-redwolves  ;
   [
    set shape "wolf"
    set color red
    set size 4  ; easier to see
    set energy random (2 * redwolf-gain-from-food)
    setxy random-xcor random-ycor
  ]
  create-werewolves initial-number-werewolves  ;
   [
    ...
    set color brown
    set size 2  ; easier to see
    set energy random (2 * werewolf-gain-from-food)
    setxy random-xcor random-ycor
  ]
  ...
  [
    ...
    set color blue
    set size 2  ; easier to see
    set energy random (2 * vampire-gain-from-food)
    setxy random-xcor random-ycor
  ]
...
to go
  ...
  ask redwolves [
    ...
    move
    set energy energy - 1  ; wolves lose energy as they move
    eat-sheep-red ; wolves eat a sheep on their patch
    death ; wolves die if they run out of energy
    reproduce-wolves ; wolves reproduce at a random rate governed by a slider
  ]
  ask werewolves [
    ...
    move
    set energy energy - 1  ; wolves lose energy as they move
    eat-wolves ; werewolves eat a wolves on their patch
    death ; wolves die if they run out of energy
    reproduce-wolves ; wolves reproduce at a random rate governed by a slider
  ]
  ask vampires [
    ...
    move
    set energy energy - 1  ; wolves lose energy as they move
    eat-wolves-vampire ; vampire eat a wolves on their patch
    eat-sheep-vampire ;vampire eat a sheep on their patch
    death ; wolves die if they run out of energy
    reproduce-wolves ; wolves reproduce at a random rate governed by a slider
  ]
...
to eat-red-grass  ; sheep procedure
  ; sheep eat deadlygrass and turn the patch brown
  if pcolor = red [
    set pcolor brown
    set energy (energy / 2) ;sheep lost half enrergy when eating the deadly grass
  ]
end
...
to eat-sheep-red  ; wolf procedure
  let prey one-of sheep-here                    ; grab a random sheep
  if prey != nobody  [                          ; did we get one? if so,
    ask prey [ die ]                            ; kill it, and...
    set energy energy + redwolf-gain-from-food     ; get energy from eating
  ]
end
to eat-sheep-vampire  ; wolf procedure
  let prey one-of sheep-here                    ; grab a random sheep
  if prey != nobody  [                          ; did we get one? if so,
    ask prey [ die ]                            ; kill it, and...
    set energy energy + vampire-gain-from-food     ; get energy from eating
  ]
end
to eat-wolves  ; werewolf procedure
  ...
  if prey != nobody  [                          ; did we get one? if so,
    ask prey [ die ]                            ; kill it, and...
    set energy energy + werewolf-gain-from-food     ; get energy from eating
  ]
end
to eat-wolves-vampire  ; werewolf procedure
  let prey one-of wolves-here                    ; grab a random sheep
  if prey != nobody  [                          ; did we get one? if so,
    ask prey [ die ]                            ; kill it, and...
    set energy energy + vampire-gain-from-food     ; get energy from eating
...
"redwolves" 1.0 0 -2674135 true "" "plot count redwolves"
"werewolves" 1.0 0 -6459832 true "" "plot count werewolves"
